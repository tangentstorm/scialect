...
export class GeminiTui extends TuiAgent {
  constructor(target: string) {
    super(target);
  }

  async isPromptBlank(): Promise<boolean> {
    // Capture as plain text (same approach as Claude and Codex)
    ...
    if (promptLine) {
      const pos = promptLine.indexOf('>');
      const after = promptLine.slice(pos + 1);
      return after.trim() === '';
    }
